...
final class PolicySingleSourceTests: XCTestCase {
    ...
    func testEnvironmentAndPipelineSeeTheSamePolicy() throws {
        ...
        try withDeemedMode(.positionAverage) {
            ...
            XCTAssertEqual(env.policies.id, "cointax-v1.1")
        }
    ...
    func testNonDefaultPolicyBumpsBundleID() throws {
        try withDeemedMode(.positionAverage) {
            XCTAssertEqual(PolicyBundle.current.id, "cointax-v1.1", "기본값은 id 를 바꾸지 않는다")
        }
        try withDeemedMode(.perLot) {
            XCTAssertEqual(PolicyBundle.current.id, "cointax-v1.1+deemed_perLot")
        ...
        XCTAssertEqual(PolicyBundle.v1Default.id, "cointax-v1.1")
