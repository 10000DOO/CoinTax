...
enum TaxOpenQuestions {
    static let all: [TaxOpenQuestion] = [
        ...
        .init(
            ...
            currentAssumption: "**1원 단위 사사오입**(반올림)을 씁니다. 엔진과 검증기가 같은 규칙을 공유합니다.",
            whatToAsk: """
            가상자산 기타소득 산출세액과 지방소득세를 신고서에 적을 때 \
            원 단위 처리(사사오입 / 10원 미만 절사 등)의 기준을 알고 싶습니다.
            """,
            impact: "표시·신고 금액이 최대 수십 원 단위로 달라집니다. 예상 세액 용도에서는 실질 차이가 거의 없습니다.",
            basis: "docs/IMPLEMENTATION.md §6.5 (v1 잠금: plain half-up) · 국고금관리법상 절사 규정 확인 필요",
            switchPoint: "코드: RoundingPolicy 구현체 교체"
        ),
        .init(
            id: "TQ-04",
            kind: .needsConfirmation,
        ...
        .init(
            ...
            kind: .needsConfirmation,
            weight: .medium,
            title: "USDT를 미국 달러 1:1로 보는 가정",
            currentAssumption: "**1 USDT = 1 USD** 로 보고 거래일 원/달러 기준환율로 환산합니다.",
            whatToAsk: """
            USDT 등 외국통화 연동 기축가상자산을 원화로 환산할 때 \
            액면 1:1(1 USDT = 1 USD)로 보아 원/달러 환율만 적용하는 것이 맞는지, \
            아니면 실제 USDT 시세(페그 이탈 구간 포함)를 반영해야 하는지 알고 싶습니다.
            """,
