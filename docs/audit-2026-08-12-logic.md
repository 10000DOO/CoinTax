...
한계: 지갑에서 다시 거래소로 들여올 때는 그 거래소의 입금 기록과 **수동 연결**해야 한다
(자동 후보는 지갑 쪽에 출금 기록이 없어 잡히지 않는다).
...
| `PersonalWalletTests` 5건 | 지갑 이동 시 원가·의제취득가 보존 · 대조군(소멸) · 네트워크 수수료 안분 · 연결 해제 시 생성 이벤트 삭제 · 잘못된 입력 거부 |
...
전체: 144 tests, 0 failures (`scripts/smoke.sh`).
...
2. **지갑 → 거래소 재입고** — 지갑 쪽에는 출금 기록이 없어 자동 후보가 안 잡힌다. 「직접 연결하기」로 이어야 한다.
3. **개인지갑 안에서 일어난 일** — 지갑에서 지갑으로 옮기거나 디파이에 쓴 기록은 앱이 알 수 없다.
4. **2027-01-01 0시 시가** — 사용자 입력이 필요하다 (환율은 ECOS 인증키로 자동).
5. **과세 시행일 하드코딩** — `TaxTime.taxStartDate`. 유예되면 의제 스냅샷 시점까지 함께 움직여야 한다 (TQ-15).
6. **거래소 잔고 열 대조(V-BAL)** — 1차 감사 §5 그대로. 이력 공백을 지금은 내부 정합성으로만 추정한다.
