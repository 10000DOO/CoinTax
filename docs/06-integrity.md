...
| V-DEM-06 | 리포트의 「다른 방식으로 계산하면」 의제취득가 **방향** (매입 건별 ≥ 보유 평균). 사용자가 TQ-01 을 고르는 근거인데 지금까지 검산이 없었다 (5차 감사 회차 21) | ✅ |
...
| V-TAX-07 | 요약의 금액이 **값이긴 한가** (`Decimal` 자릿수 넘침 → 조용히 `NaN`). `NaN` 은 모든 비교를 통과하고 `max(0, NaN)=0` 이라 **세액이 0원으로 접힌다** (5차 감사 회차 34) | ✅ |
...
| V-IMP-06 | 한 원본만 **한참 일찍 끝나는가** — 조회기간을 짧게 받아오면 그 뒤 거래가 통째로 빠지는데, **명세서 마지막 뒤는 어떤 검사로도 확인할 수 없다** (5차 감사 회차 14·15 실측) | Warning |
