...
> 상태: `구현중` · 갱신: 2026-08-13 · 브랜치: `main` · 다음 액션: **WO-4 (계산 결과를 거주자별 원가 풀로 전환)** — 6장 WO-4 를 그대로 실행한다. 그 전까지 세액은 아직 옛 규칙(계정별 이동평균/선입선출)으로 나온다
...
| 4 | **A-1 총평균법 — 결과를 풀로 전환** | **WO-4 `[ ]` ← 다음** | 골든 재계산이 전부 손계산으로 설명됨 |
| 5 | A-2 50% 의제 | WO-5 `[ ]` | |
| 6 | C 백서 마무리 | WO-6 `[ ]` | |
| 7 | B 신고 안내 | WO-7 `[ ]` | |
